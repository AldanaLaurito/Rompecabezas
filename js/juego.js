...
var instrucciones = ["Reordene la imagen.","Mueva la pieza roja para mover el resto de las piezas.","Se permite cualquier movimiento dentro de los bordes."];
...
function mostrarInstrucciones(instrucciones) {
	...
	var longInstruccion = instrucciones.length;
	var instruccionAct = "";
	
	for(var i=0; i<longInstruccion; i++){
		instruccionAct = instrucciones[i];
		mostrarInstruccionEnLista(instruccionAct,"instrucciones");		
	}
}

/* Función que agrega la última dirección al arreglo de movimientos
y utilice actualizarUltimoMovimiento para mostrarlo en pantalla */
function guardarUltimoMovimiento(ultMovimiento){
	movimientos.push(ultMovimiento);
	actualizarUltimoMovimiento(ultMovimiento);
}
...
function chequearSiGano() {
	...
	for(var i=0; i<3; i++){
		var arrayGrilla = grilla[i];
		
		var arrayGanadora = grillaGanardora[i];
		
		for(var j=0; j<3; j++){
			if(arrayGrilla[j] != arrayGanadora[j]){
				return false;
			}
		}
	}
...
function mostrarCartelGanador() {
	var elementoPieza1 = document.getElementById("popup");
...
function posicionValida(fila, columna) {
	return (rango(fila) && rango(columna));
}
...
function moverEnDireccion(direccion) {
  ...
  if (direccion === codigosDireccion.ABAJO) {
	...
	//En codigo original era signo menos
  ...
  // Mueve pieza hacia arriba, reemplazandola con la blanca
	...
	//En codigo original era signo mas
    nuevaFilaPiezaVacia = filaVacia - 1;
    nuevaColumnaPiezaVacia = columnaVacia;
  }
  ...
  las funciones posicionValida, intercambiarPosicionesGrilla y actualizarPosicionVacia */
    ...
    if (posicionValida(nuevaFilaPiezaVacia, nuevaColumnaPiezaVacia)) {
		...
		//Se agrega la dirección del movimiento al arreglo de movimientos
		guardarUltimoMovimiento(direccion);

	//Esto, el alert, lo agregue yo
    }else if (!mezclaPiezas){
		alert("El movimiento que intenta realizar no es posible.")
	}
}
